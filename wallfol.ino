...
int errL, errR;
  ...
  while(1){
    ...
    if(ms > msref){
      ...
      if(rung) rung--; // running
      acq_sensors();
      errL=range[Lft]-120;   // far:pos near:neg   -78..135;
      errL/=6;
      errR=range[Rgt]-120;
      errR/=6;
      ledBlink();
      ...
      switch(sq_m){
       ...
       case(2): *p_trc=range[Ctr]; if(p_trc==&trace[9]) p_trc=trace; else p_trc++; 
                  if(range[Ctr] < 80){wmove(0x0,0x0); sq_m=3; break;} // stop by front
                  if(errL){
                    spdL=0x18-errL;
                    spdR=0x18+errL;
       ...
       case(3): if(Halted){
                   for(i=0; i<9; i++){
                    Serial.println(*p_trc);
                    if(p_trc==&trace[9]) p_trc=trace; else p_trc++;
                  }
                  Serial.print("    ");
                  Serial.println(*p_trc);                  
                  i=0; sq_m=4;
                 }break;
        case(4): Serial.println(range[Ctr]);
                 i++;
                 if(i==20){wmove(0x0,0x0); sq_m=0;}
                 break;
